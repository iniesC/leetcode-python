...
'''
Using DFS and Graph bipartite logic
...
def dfs(current_user, adjList, current_group, groups):
    # group the current user
    groups[current_user] = current_group
    next_group =  1 - groups[current_user]

    for next_user in adjList[current_user]:
        if next_user not in groups and not dfs(next_user, adjList, next_group, groups):
            return False
        else:
            if groups[next_user] == groups[current_user]: #check neighbor not in same group
                return False
    
    return True


def isBiPartite(N, dislikes):
    ...
    for d in dislikes:
        src, dest = d[0], d[1]
        adjList[src].append(dest) #both dislike each other
        adjList[dest].append(src)

    starting_group = 0
    groups = {}
    for user in range(1,N+1):
        if user not in groups and not dfs(user,adjList,starting_group, groups):
                return False
...
def main():
    ...
    #test 4
    N = 50
    dislikes = [[21,47],[4,41],[2,41],[36,42],[32,45],[26,28],[32,44],[5,41],[29,44],[10,46],
                ...
                [26,43],[15,46],[42,45],[18,42],[25,42],[38,41],[32,39],[6,30],[29,33],[34,37],
                [26,38],[3,22],[18,47],[42,48],[22,49],[26,34],[22,36],[29,36],[11,25],[41,44],
                [6,46],[13,22],[11,16],[10,37],[42,43],[12,32],[1,48],[26,40],[22,50],[17,26],
