...
'''
Using BFS and Graph bipartite logic
...
def bfs(starting_user, adjList, current_group, groups):
    
    q = []
    q.append(starting_user)
    groups[starting_user] = current_group
    while q:
        current_user = q.pop(0)
        next_group =  1 - groups[current_user]

        for next_user in adjList[current_user]:
            if next_user not in groups:
                groups[next_user] = next_group
                q.append(next_user)
            else:
...
def isBiPartite(N, dislikes):
    ...
    for user in range(1,N+1):
        if user not in groups and not bfs(user,adjList,starting_group, groups):
                return False
